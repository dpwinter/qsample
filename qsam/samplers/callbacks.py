# AUTOGENERATED! DO NOT EDIT! File to edit: 05b_samplers.callbacks.ipynb (unless otherwise specified).
...
class CallbackList:
    ...
    def on_sample_begin(self):
        for callback in self.callbacks:
            callback.on_sample_begin(sampler=self.sampler)

    def on_sample_end(self):
        for callback in self.callbacks:
            callback.on_sample_end(sampler=self.sampler)
...
class PlotStats(Callback):
    ...
    def on_sampler_end(self, sampler):
        ...
        if len(stats) == 2:
            ax.errorbar(self.sample_range, stats[0], fmt='--', c="black", yerr=stats[1], label="Direct MC")
        else:
            p_L_low, std, delta, delta_std = stats
            p_L_up = p_L_low + delta
            err = np.sqrt(std**2 + delta_std**2)
            ...
            ax.fill_between(self.sample_range, p_L_up-err, p_L_up+err, alpha=0.2)
...
class RelStdTarget(Callback):
    ...
    def on_sample_end(self, sampler):
        if hasattr(sampler, 'p_idx'):
            ...
            p_L, std, delta, _ = sampler.stats(const="Aws_pmax")
...
class StatsPerSample(Callback):
    ...
    def on_sampler_end(self, **kwargs):
        ...
        labels = ['$p_L$', 'std', '$\delta$', 'std[$\delta$]']
